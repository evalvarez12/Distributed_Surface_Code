...
def bell_pair(p):
    """
    Return a noisy |phi+> Bell pair of the form 
    $(1-p)*\Phi^{+} + p/3 * ( \Phi^{-} + \Psi^{+} + \Psi^{-})$
...
def bell_pair_psi(p):
    """
    Return a noisy |psi+> Bell pair of the form 
    $(1-p)*\Phi^{+} + p/3 * ( \Phi^{-} + \Psi^{+} + \Psi^{-})$
...
def bell_pair_click(eta, theta):
    ...
    -----------
    eta : ???
        (scalar) detection efficiency
    theta : ???
        (scalar) determines how the initial qubits are initialized
