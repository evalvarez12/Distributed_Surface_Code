...
import faulttolerance.decomposition.tools.p_success as ps
import faulttolerance.decomposition.tools.ghz_correction as gc
...
class Blocks:
    ...
    def __init__(self, ps, pm, pg, eta, a0, a1, theta):
        ...
        # Tim: it is best to take these values to some external file (e.g. JSON)
...
########################################################################
# Functions that deal with generating noisy Bell states
########################################################################
    ...
    def _generate_bell_single_click(self):
        ...
        Returns
        -------
        duration, bell : float, qutip.qobj.Qobj
            Time it took to generate the state, the quantum state
        """
        ...
        # Tim: do you have a reference to this number?
        ...
        duration = self.time_lookup["bell_pair"] * attempts
    ...
    def _success_number_of_attempts(self, p_success):
        """
        Tim: rename to 'draw_sample_from_CDF_geometric'?
        """
    ...
    def _distribution(self, p, n):
        ...
        ----------
        p : float
            Probability that entanglement generation attempt
        n : Numpy Array of float
            Numpy Array of numbers of attempts

        Returns
        -------
        Numpy Array of floats
            Numpy Array of numbers p*(1-p)^n, which is the probability that
            precisely n attempts are needed before first successful
            attempt.
        """
...
########################################################################
# Functions that deal with appending a specified state to a bigger state
########################################################################
...
###################################################################
    ...
    def _get_two_qubit_gates(self, N, controls, targets, sigma):
        """
        Applies a Control-Sigma gate, where Sigma is either X or Z.

        Parameters
        ----------
        N : int
            Length of the gate (i.e. there are N-1 control qubits).
        controls : list of ???
        targets : list of ???
        sigma : string
            Either "X" or "Z"
...
###################################################################
# Several methods for creating a Bell pair
###################################################################
    ...
    def start_bell_pair(self, rho=None):
        ...
        Returns
        -------
        1, check list, bell state : int, Collections.Counter, qutip.Qobj
        """
        self._reset_check()
        time, bell = self._generate_bell_single_click()
        self.check["time"] += time
    ...
    def add_bell_pair(self, rho):
        # Tim: rename to `append_single_click_bell_pair`
